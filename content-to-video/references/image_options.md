...
> 比例依据：横屏图片槽是 860×608（√2:1 ≈ 1.414）横版卡片，`object-fit:contain` 填充下 4:3 图填充率约 94%（两侧各 ~25px），方图（1:1）约 71%（两侧各 ~126px），16:9 约 80%（上下各 ~62px）——√2:1 对主流素材（4:3 生图 / 16:9 搜图与视频）是均衡折中，优于旧 1.229 槽（4:3 92% / 16:9 69%）。竖屏只有 verse 模式（`--aspect portrait`，1080×1440）、图片槽是 4:3 横版 980×735（4:3 图 100% 填充——1:1 方图加 300px 句子流在 1440 高度里放不下）——默认横屏优先，`--aspect both` 双画幅时 4:3 是两边兼顾的折中（与方式 C 图表画布同一比例约定）。
