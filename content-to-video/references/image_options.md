...
> 比例依据：横屏图片槽是 860×700（≈1.23:1）横版卡片，`object-fit:contain` 填充下 4:3 图填充率约 92%（上下各 ~27px 模糊底），方图（1:1）约 81%（两侧各 ~80px），16:9 约 69%（上下各 ~108px）。竖屏家族只有 verse 模式、图片槽随画幅取形：标准竖屏（1080×1920）是正方形 980×980（方图填充率 100%，4:3 约 75%、16:9 约 56%），紧凑竖屏 portrait（1080×1440）是 4:3 横版 980×735（4:3 图 100% 填充——1:1 方图加 300px 句子流在 1440 高度里放不下）——默认横屏优先，`--aspect both` 双画幅时 4:3 是两边兼顾的折中（与方式 C 图表画布同一比例约定）。
