...
> **路径 / 缓存等内部机制 →** 排查诡异的路径或缓存问题时，先看本文件末尾的「内部机制速览」节（数据流全景、路径解析速记、缓存与断点续跑）；日常制作无需看，第 3 步记得加 `--resume` 即可。
...
## 附：内部机制速览（数据流 / 路径解析 / 缓存）

排查诡异的路径/缓存问题时先看这一节；日常制作不需要——第 3 步记得加 `--resume` 就够。
...
- 其余路径规则见本文件条目 #9（images.json 路径相对 HTML 目录）、#2（total_duration 用 ffmpeg 实测）与 SKILL.md 第 5 步"音频引用"（音频自动拷进项目根），不在此重复。
